...
pub struct UpdateCounters {
    ...
    pub nodes_updated_from_layout: u32,
...
bitflags! {
    ...
    struct DirtyState : u16 {
        ...
        const DescendantHasDomDamage = 0b0001;
        /// This node has unresolved damage from the DOM tree.
        const HasDomDamage = 0b0010;
...
impl AccessibilityTree {
    ...
    fn mark_nodes_and_ancestors_dirty(
...

        {
            ...
            first_node.dirty_state |= DirtyState::HasDomDamage;
            ...
            common_ancestors.extend(first_node.ancestors().map(|ancestor| {
                ...
                ancestor.dirty_state |= DirtyState::DescendantHasDomDamage;
                ancestor.id
            }));
        ...
        let mut truncate_ancestors = |node: &AccessibilityNode| -> bool {
            if node.dirty_state.descendant_has_dom_damage() {
        ...
        for node_id in dirty_node_ids {
            ...
            node.dirty_state |= DirtyState::HasDomDamage;
        ...
        for ancestor_id in common_ancestors {
            ...
            ancestor.borrow_mut().dirty_state -= DirtyState::DescendantHasDomDamage;
        }
...
impl AccessibilityNode {
    ...
    fn update_subtree<'update>(
        ...
        if let Some(dom_node) = dom_node {
            ...
            self.dirty_state -= DirtyState::HasDomDamage;
        }

        hidden |= self.is_hidden();

        if self.dirty_state.descendant_has_dom_damage() ||
        ...
        self.dirty_state -= DirtyState::DescendantHasDomDamage;
    ...
    fn update_ancestors(
        ...
        for node in self.ancestors() {
            ...
            node.dirty_state -= DirtyState::DescendantHasDomDamage;
    ...
    fn update_children_from_dom_node<'update>(
        ...
        for dom_child in remaining_dom_children {
            ...
            self.dirty_state
                .propagate_descendant_has_dom_damage(child.dirty_state);
        }
    ...
    fn update_node_from_layout(
        ...
        update.counters.nodes_updated_from_layout += 1;
    ...
    fn compute_damage(
        ...
        if self.dirty_state.has_dom_damage() {
...
impl DirtyState {
    ...
    fn has_dom_damage(&self) -> bool {
        self.contains(DirtyState::HasDomDamage)
    }

    fn descendant_has_dom_damage(&self) -> bool {
        self.contains(DirtyState::DescendantHasDomDamage)
    }

    fn propagate_descendant_has_dom_damage(&mut self, child_dirty_state: DirtyState) {
        if child_dirty_state.self_or_descendant_has_dom_damage() {
            self.insert(DirtyState::DescendantHasDomDamage)
        }
    }

    fn self_or_descendant_has_dom_damage(&self) -> bool {
        self.intersects(DirtyState::HasDomDamage | DirtyState::DescendantHasDomDamage)
    }
}
